...
public class BitmapFontCache {
	...
	private ColorTagMarkdownParser markdownParser = new ColorTagMarkdownParser();
	private TextMarkdown markdown = new TextMarkdown();
	...
	public void tint (Color tint) {
		...
		if (textChanged || oldTint != floatTint) {
			textChanged = false;
			oldTint = floatTint;
            markdown.tint(this, tint, floatTint);
		}
	}
			...
			while (start < end) {
				...
				if (ch == '[' && font.markupEnabled) {
					if (!(start < end && str.charAt(start) == '[')) { // non escaped '['
						start += markdownParser.parseAndAddChunk(markdown, str, charsCount, start, end) + 1;
						continue;
					}
					start++;
				}
	...
	public TextMarkdown getMarkdown() {
		return markdown;
	}

	public void setMarkdown(TextMarkdown markdown) {
